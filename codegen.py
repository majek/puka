...
def print_encode_properties(c):
    print("%s_PROPS_SET = {" % (c.name.upper(),))
    for f in c.fields:
    ...
    for i, f in enumerate(c.fields):
        ...
        if len(fields.fields) > 1:
            print(' ' * 8 + "lambda val: join_as_bytes((")
            fields.do_print(' ' * 16, '%s')
            print(' ' * 8 + ')) ),')
        else:
    ...
    print()
    print("    for key in %s_PROPS_SET & set(futils.viewkeys(props)):" % \
          (c.name.upper(),))
    print("        i, f, fun = enc[key]")
    print("        flags |= f")
    print("        pieces[i] = fun(props[key])")
    print("")
    print("    return (0x02, join_as_bytes((")
    print("        pack('!HHQH',")
    print("              %s, 0, body_size, flags)," % (c.u,))
    print("        join_as_bytes(pieces),")
    print("        ))")
    print("        )")
...
def split_headers(user_headers, properties_set):
    props = {}
    headers = {}
    for key, value in futils.viewitems(user_headers):
