...
def decode_connection_start(data, offset):
    ...
    (frame['version_major'],
     frame['version_minor']) = struct.unpack_from('!BB', data, offset)
    offset += 1+1
...
def decode_connection_tune(data, offset):
    ...
    (frame['channel_max'],
     frame['frame_max'],
     frame['heartbeat']) = struct.unpack_from('!HIH', data, offset)
    offset += 2+4+2
    return frame, offset
...
def decode_basic_properties(data, offset):
    ...
    if (flags & 0x1000): # 1 << 12
        (props['delivery_mode'],) = struct.unpack_from('!B', data, offset)
        offset += 1
    if (flags & 0x0800): # 1 << 11
        (props['priority'],) = struct.unpack_from('!B', data, offset)
        offset += 1
...
def encode_connection_start_ok(client_properties, mechanism, response, locale):
    ...
    return ( (0x01,
              ''.join((
                ...
                struct.pack('!B', len(mechanism)),
                mechanism,
                struct.pack('!I', len(response)),
                response,
                struct.pack('!B', len(locale)),
                locale,
              ))
           ), )
...
def encode_connection_secure_ok(response):
    return ( (0x01,
              ''.join((
                struct.pack('!II', METHOD_CONNECTION_SECURE_OK, len(response)),
                response,
              ))
           ), )
...
def encode_connection_tune_ok(channel_max, frame_max, heartbeat):
    return ( (0x01,
                struct.pack('!IHIH', METHOD_CONNECTION_TUNE_OK, channel_max, frame_max, heartbeat),
           ), )
...
def encode_connection_close_ok():
    return ( (0x01,
                struct.pack('!I', METHOD_CONNECTION_CLOSE_OK),
           ), )
...
def encode_channel_flow(active):
    return ( (0x01,
                struct.pack('!IB', METHOD_CHANNEL_FLOW, (active and 0x1 or 0)),
           ), )

# active=None
def encode_channel_flow_ok(active):
    return ( (0x01,
                struct.pack('!IB', METHOD_CHANNEL_FLOW_OK, (active and 0x1 or 0)),
           ), )
...
def encode_channel_close_ok():
    return ( (0x01,
                struct.pack('!I', METHOD_CHANNEL_CLOSE_OK),
           ), )
...
def encode_exchange_declare(exchange, type_, passive, durable, auto_delete, internal, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_EXCHANGE_DECLARE, 0, len(exchange)),
                exchange,
                struct.pack('!B', len(type_)),
                type_,
                struct.pack('!B', (passive and 0x1 or 0) | (durable and 0x2 or 0) | (auto_delete and 0x4 or 0) | (internal and 0x8 or 0)),
                arguments_raw,
              ))
           ), )
...
def encode_exchange_delete(exchange, if_unused):
    return ( (0x01,
              ''.join((
                ...
                exchange,
                struct.pack('!B', (if_unused and 0x1 or 0)),
              ))
           ), )
...
def encode_exchange_bind(destination, source, routing_key, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_EXCHANGE_BIND, 0, len(destination)),
                destination,
                struct.pack('!B', len(source)),
                source,
                struct.pack('!B', len(routing_key)),
                routing_key,
                "\x00",
                arguments_raw,
              ))
...
def encode_exchange_unbind(destination, source, routing_key, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_EXCHANGE_UNBIND, 0, len(destination)),
                destination,
                struct.pack('!B', len(source)),
                source,
                struct.pack('!B', len(routing_key)),
...
def encode_queue_declare(queue, passive, durable, exclusive, auto_delete, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_QUEUE_DECLARE, 0, len(queue)),
                queue,
                struct.pack('!B', (passive and 0x1 or 0) | (durable and 0x2 or 0) | (exclusive and 0x4 or 0) | (auto_delete and 0x8 or 0)),
                arguments_raw,
              ))
           ), )
...
def encode_queue_bind(queue, exchange, routing_key, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_QUEUE_BIND, 0, len(queue)),
                queue,
                struct.pack('!B', len(exchange)),
                exchange,
                struct.pack('!B', len(routing_key)),
                routing_key,
                "\x00",
...
def encode_queue_delete(queue, if_unused, if_empty):
    return ( (0x01,
              ''.join((
                ...
                queue,
                struct.pack('!B', (if_unused and 0x1 or 0) | (if_empty and 0x2 or 0)),
              ))
           ), )
...
def encode_queue_unbind(queue, exchange, routing_key, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_QUEUE_UNBIND, 0, len(queue)),
                queue,
                struct.pack('!B', len(exchange)),
                exchange,
                struct.pack('!B', len(routing_key)),
...
def encode_basic_consume(queue, consumer_tag, no_local, no_ack, exclusive, arguments):
    ...
    return ( (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_BASIC_CONSUME, 0, len(queue)),
                queue,
                struct.pack('!B', len(consumer_tag)),
                consumer_tag,
                struct.pack('!B', (no_local and 0x1 or 0) | (no_ack and 0x2 or 0) | (exclusive and 0x4 or 0)),
                arguments_raw,
              ))
           ), )
...
def encode_basic_publish(exchange, routing_key, mandatory, immediate, user_headers, body, frame_size):
    ...
    return [ (0x01,
              ''.join((
                struct.pack('!IHB', METHOD_BASIC_PUBLISH, 0, len(exchange)),
                exchange,
                struct.pack('!B', len(routing_key)),
                routing_key,
                struct.pack('!B', (mandatory and 0x1 or 0) | (immediate and 0x2 or 0)),
              ))
           ),
...
def encode_basic_get(queue, no_ack):
    return ( (0x01,
              ''.join((
                ...
                queue,
                struct.pack('!B', (no_ack and 0x1 or 0)),
              ))
           ), )
...
def encode_basic_ack(delivery_tag, multiple):
    return ( (0x01,
                struct.pack('!IQB', METHOD_BASIC_ACK, delivery_tag, (multiple and 0x1 or 0)),
           ), )

# delivery_tag=None requeue=True
def encode_basic_reject(delivery_tag, requeue):
    return ( (0x01,
                struct.pack('!IQB', METHOD_BASIC_REJECT, delivery_tag, (requeue and 0x1 or 0)),
           ), )

# requeue=False
def encode_basic_recover_async(requeue):
    return ( (0x01,
                struct.pack('!IB', METHOD_BASIC_RECOVER_ASYNC, (requeue and 0x1 or 0)),
           ), )
